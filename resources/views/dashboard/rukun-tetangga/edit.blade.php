					...
					<div class="card-body">
						<div class="form-row">
                            <div class="form-group col-md-6">
                                ...
                                <input type="text" class="form-control @error('number') is-invalid @enderror" name="number" id="number" value="{{old('number', $rt->number)}}">
                                @error('number')
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <select class="form-control selectRW @error('rw') is-invalid @enderror" name="rw" id="rw">
                                    <option></option>
                                    @foreach ($rw as $no => $r)
                                    <option value="{{ $r->id }}" @if(old('rw', $r->id) == $rt->rukun_warga_id) selected @endif>RW {{ $r->number }}</option>
                                    @endforeach
							    </select>
                                @error('rw')
                                    <span class="invalid-feedback" role="alert">
